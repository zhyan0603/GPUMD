...
// ============================================================================
// ACNEP OPTIMIZATION: Neighbor list with distance caching
// ============================================================================
// This kernel pre-computes neighbor lists AND caches distances to avoid
...
__global__ void gpu_find_neighbor_list_with_cache(
  ...
  float* y12_radial,
  float* z12_radial,
  float* r_radial,      // NEW: Cache distances for radial
  float* x12_angular,
  float* y12_angular,
  float* z12_angular,
  float* r_angular)     // NEW: Cache distances for angular
{
        ...
        for (int ib = 0; ib < num_cell[1]; ++ib) {
          for (int ic = 0; ic < num_cell[2]; ++ic) {
            ...
            if (distance_square < rc_radial * rc_radial) {
              ...
              r_radial[count_radial * N + n1] = distance;  // NEW: Store distance
              count_radial++;
            }
            if (distance_square < rc_angular * rc_angular) {
              ...
              r_angular[count_angular * N + n1] = distance;  // NEW: Store distance
              count_angular++;
            }
          }
        }
...
// ============================================================================
// ACNEP OPTIMIZATION: Descriptor kernels with cached distances
// ============================================================================
// These versions use pre-computed distances to avoid redundant sqrt() calls

static __global__ void find_descriptors_radial_cached(
  const int N,
  const int* g_NN,
  const int* g_NL,
  const NEP::ParaMB paramb,
...
static __global__ void find_descriptors_angular_cached(
  const int N,
  const int* g_NN,
  const int* g_NL,
  const NEP::ParaMB paramb,
  ...
  int device_in_this_iter)
{
  ...
  for (int device_id = 0; device_id < device_in_this_iter; ++device_id) {
    ...
    // ACNEP OPTIMIZATION: Use pre-computed geometry if available
    bool use_cache = dataset[device_id].precomp_geom.is_cached;
    
    if (calculate_neighbor && !use_cache) {
      // Original path: compute neighbor list on the fly
    ...
    // If use_cache, neighbor list computation is SKIPPED (major speedup!)

    if (use_cache) {
      // ACNEP OPTIMIZED PATH: Use cached distances to avoid sqrt()
      find_descriptors_radial_cached<<<grid_size, block_size>>>(
        dataset[device_id].N,
        dataset[device_id].precomp_geom.NN_radial.data(),
        dataset[device_id].precomp_geom.NL_radial.data(),
        paramb,
        annmb[device_id],
        dataset[device_id].type.data(),
        dataset[device_id].precomp_geom.x12_radial.data(),
        dataset[device_id].precomp_geom.y12_radial.data(),
        dataset[device_id].precomp_geom.z12_radial.data(),
        dataset[device_id].precomp_geom.r_radial.data(),  // Cached distances!
        nep_data[device_id].descriptors.data());
      GPU_CHECK_KERNEL

      find_descriptors_angular_cached<<<grid_size, block_size>>>(
        dataset[device_id].N,
        dataset[device_id].precomp_geom.NN_angular.data(),
        dataset[device_id].precomp_geom.NL_angular.data(),
        paramb,
        annmb[device_id],
        dataset[device_id].type.data(),
        dataset[device_id].precomp_geom.x12_angular.data(),
        dataset[device_id].precomp_geom.y12_angular.data(),
        dataset[device_id].precomp_geom.z12_angular.data(),
        dataset[device_id].precomp_geom.r_angular.data(),  // Cached distances!
        nep_data[device_id].descriptors.data(),
        nep_data[device_id].sum_fxyz.data());
      GPU_CHECK_KERNEL
    } else {
      // Original path: recompute distances in descriptor kernels
    ...
    // ACNEP: Use cached geometry for force computation when available
    if (use_cache) {
      // OPTIMIZED PATH: Use pre-computed cached geometry
      ...
      // Original path: use dynamically computed geometry
      find_force_radial<<<grid_size, block_size>>>(
        dataset[device_id].N,
        nep_data[device_id].NN_radial.data(),
        nep_data[device_id].NL_radial.data(),
